...
Important note for this fork
-----------------------------

This fork introduces two key differences from the original work.

1. The first one is the compatibility with `pytest-bdd` v5 instead of just `pytest-bdd` v6.
2. The second one is this forked version doesn't hide the other json reports generated by `pytest` (e.g., the `cucumber.json` file).

To install what is published in this specific fork, please use the plugin version `1.0.3b0`.
...
::
    ...
    $ pip install pytest-bdd-report=="1.0.3b0"
